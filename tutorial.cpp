...
int run(DBClientBase* conn) {
    ...
    conn->remove("tutorial.persons", BSONObj());
    ...
    cout << " all data: \n" 
           << conn->findOne("tutorial.persons", BSONObj()) << endl;
    cout << "----------" << endl;

    //更新指定的属性值
    const string TEST_NS = "tutorial.persons";
    BSONObj res = conn->findOne(TEST_NS, BSONObjBuilder().append("name", "joe").obj());
        conn->update("tutorial.persons", res, BSON("$set" << BSON("age" << 21)));
